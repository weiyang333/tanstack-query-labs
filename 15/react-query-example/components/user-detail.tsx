...
import { ArrowLeft, AlertCircle, User, Mail, Calendar, MessageSquare } from "lucide-react"
...
import { UserComment } from '../app/api/types'
...
// 获取用户评论
export async function fetchUserComments(userId: number): Promise<UserComment[]> {
  // 真实环境中，这里应该是实际的API调用
  const response = await fetch(`/api/users/${userId}/comments`)

  if (!response.ok) {
    throw new Error(`获取用户评论失败: ${response.status}`)
  }
        ...
        <CardContent>
          {isCommentsLoading ? (
            ...
            <Alert variant="destructive">
              <AlertCircle className="h-4 w-4" />
              <AlertDescription>
                {commentsError instanceof Error ? commentsError.message : "获取评论失败"}
              </AlertDescription>
            </Alert>
          ) : comments?.length ? (
              ...
              {comments.map(comment => (
                <div key={comment.id} className="border rounded-lg p-3 bg-muted/30">
                  ...
                  <div className="flex justify-between items-center mt-2">
                    ...
                    <Badge variant="outline" className="text-xs">评论 #{comment.id}</Badge>
                  </div>
                </div>
              ))}
            </div>
